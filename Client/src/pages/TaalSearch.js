...
const TaalSearch = () => {
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/taals/search?name=${encodeURIComponent(searchQuery)}&useAI=${useAI}&aiProvider=${aiProvider}`);
      setTaal(response.data);
      toast.success(`Taal data ${useAI ? `researched using ${aiProvider.toUpperCase()} AI` : 'scraped from web'} successfully`);
    } catch (error) {
