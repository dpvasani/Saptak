...
const ArtistSearch = () => {
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/artists/search?name=${encodeURIComponent(searchQuery)}&useAI=${useAI}&aiProvider=${aiProvider}`);
      setArtist(response.data);
      toast.success(`Artist data ${useAI ? `researched using ${aiProvider.toUpperCase()} AI` : 'scraped from web'} successfully`);
    } catch (error) {
